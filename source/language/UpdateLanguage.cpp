...
int DownloadAllLanguageFiles()
{
	...
	{
		ShowError(tr("Could not create language files path %s"), Settings.languagefiles_path);
		return -1;
	}

	if(!IsNetworkInit())
	{
		ShowError(tr("Network connection disabled"));
		return -2;
	}
	char fullURL[300];
...
int UpdateLanguageFiles()
{
	...
	{
		if(WindowPrompt(tr("Update Wizard"), tr("No language files found. Download new language files?"), tr("Yes"), tr("No")))
