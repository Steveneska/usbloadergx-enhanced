...
int SelectPartitionMenu()
{
	...
	for (int cnt = 0; cnt < DeviceHandler::GetUSBPartitionCount(); cnt++)
	{
		...
		else
		{
			...
			options.SetValue(counter, tr("cannot be formatted" ));
		}
		counter++;
	}
	...
	while (!ExitSelect)
	{
		...
		if (ret >= 0)
		{
			if (strcmp(options.GetValue(ret), tr("cannot be formatted" )) != 0)
			{
				choice = ret;
				ExitSelect = true;
			}
		...
		{
			choice = WindowPrompt(tr("Power"), tr("Shutdown System?"), tr("Yes"), tr("No"));
			if (choice == 1)
				Sys_Shutdown();
		...
		{
			choice = WindowPrompt(tr("Exit"), tr("Close application and return to Wii Menu?"), tr("Yes"), tr("No"));
			if (choice == 1)
				Sys_LoadMenu();
		}
	}
