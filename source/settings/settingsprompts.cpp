...
bool MenuBackgroundMusic()
{
	...
	else
	{
		...
		{
			pathptr++;
			int choice = WindowPrompt(tr("Now Playing:"), pathptr, tr("Previous"), tr("Next"), tr("Directory"), tr("Cancel"));
			if (choice == 1)
	...
	if (BrowseDevice(entered, sizeof(entered), FB_DEFAULT))
	{
		if (!bgMusic->Load(entered))
			WindowPrompt(tr("File Error"), tr("Unsupported format. Loading default music"), tr("OK"));
		else
			ret = true;
		bgMusic->Play();
...
int MenuLanguageSelect()
{
	...
	while ( !returnhere )
	{
		...
		{
			choice = WindowPrompt(tr("Language"), tr("Loading default language"), tr("OK"), tr("Cancel") );
			if ( choice == 1 )
			{
		...
		{
			choice = WindowPrompt(tr("Update Wizard"), tr("Download language files to update?"), tr("OK"), tr("Cancel") );
			if ( choice == 1 )
			{
		...
		else if ( pathBtn.GetState() == STATE_CLICKED )
		{
			...
			if (result == 1)
			{
				...
				WindowPrompt(tr("Custom Paths"), tr("Language files path updated successfully"), tr("OK"));
			}
			pathBtn.ResetState();
		}
		...
		if (ret >= 0)
		{
			choice = WindowPrompt(tr("Language"), tr("Change language preference?"), tr("Yes"), tr("Cancel") );
			if (choice == 1)
			{
				...
				{
					WindowPrompt(tr("Error"), tr("Language file not found. Loading default language"), tr("OK"));
					Settings.LoadLanguage(NULL, CONSOLE_DEFAULT);
				}
				else
				{
