...
int DiscBrowse(const char * GameID, char * alternatedname, int alternatedname_size)
{
	...
	{
		ResumeGui();
		WindowPrompt(tr("Disc Error"), tr("An error occured while opening game disc"), tr("OK"));
		return ret;
	}
	...
	{
		ResumeGui();
		WindowPrompt(tr("Device Error"), tr("Not enough free space on storage device"), tr("OK"));
		return -1;
	}
	...
	gprintf("\n%i alternate DOL files found", options.GetLength()+1);
	if (options.GetLength() <= 0)
	{
		WindowPrompt(tr("Disc Error"), tr("No DOL file found on disc"), tr("OK"));
		return ret;
	}
	...
	while (!exit)
	{
		...
		if (ret >= 0)
		{
			choice = WindowPrompt(options.GetValue(ret), tr("Load as alternate DOL?"), tr("OK"), tr("Cancel"));
			if (choice)
			{
