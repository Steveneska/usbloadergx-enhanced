...
static int InternalShowGameInfo(struct discHdr *header)
{
	...
	{
		ShowError(tr("Could not open wiitdb.xml database file"));
		return -1;
	}
	...
	{
		XML_DB.CloseFile();
		ShowError(tr("Game information not found in database"));
		return -1;
	}
	...
	cover = new GuiImageData(imgPath); // Load full image
	if (!cover->GetImage())
	{
		delete cover;
		cover = NULL;
	...
	if(load3DCover && cover) //! Always 3D box if no cover found
	{
	...
	// Draw the input types for the selected game
	if (motionplus == 1)
	{
	...
	if (GameInfo.RatingType >= 0)
	{
		...
		ratingImg->SetPosition(-25, ratingsY);
	...
	{
		snprintf(linebuf2, sizeof(linebuf2), "%s: %s", tr("Publisher" ), GameInfo.Publisher.c_str());
		...
		publisherTxt->SetAlignment(ALIGN_LEFT, ALIGN_TOP);
		publisherTxt->SetPosition(205, 12 + indexY);
		indexY += (20 * newline);
		newline = 1;
	...
	{
		snprintf(linebuf2, sizeof(linebuf2), "%s: %s", tr("Developer" ), GameInfo.Developer.c_str());
		...
		developerTxt->SetAlignment(ALIGN_LEFT, ALIGN_TOP);
		developerTxt->SetPosition(205, 12 + indexY);
		indexY += (20 * newline);
		newline = 1;
	...
	if (GameInfo.PublishDate != 0)
	{
		...
		snprintf(linebuf2, sizeof(linebuf2), "%s: %s %02i %i", tr("Release Date" ), readableMonths[month - 1], day, year);
		releasedTxt = new GuiText(linebuf2, 16, ( GXColor ) {0, 0, 0, 255});
		if (releasedTxt->GetTextWidth() > 300) newline = 2;
		releasedTxt->SetAlignment(ALIGN_LEFT, ALIGN_TOP);
		releasedTxt->SetPosition(205, 12 + indexY);
		indexY += (20 * newline);
		newline = 1;
	...
	for (int i = GameInfo.WifiFeatureList.size()-1; i >= 0 && GameInfo.WifiFeatureList.size() > 0; --i)
	{
		...
		wifiTxt[i]->SetPosition(205, 12 + wifiY); // 215, 200 + wifiY
		wifiY -= 20;
	...
	wifiTxt[0]->SetPosition(205, 12 + wifiY); // 215, 200 + wifiY
	...
	if (GameInfo.GameID.size() != 0)
	{
		...
		idTxt->SetPosition(205, 12 + indexY);
		indexY += (20 * newline);
		newline = 1;
