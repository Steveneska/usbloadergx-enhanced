...
int WindowExitPrompt()
{
	...
	while (choice == -1)
	{
		...
		else if (btn2.GetState() == STATE_CLICKED)
		{
			...
			{
				ret = WindowPrompt(tr("Exit"), 0, tr("Homebrew Channel"), tr("Wii Menu"), tr("Reset"), tr("Cancel"));
				if (ret == 1)
					Sys_LoadHBC();
				else if(ret == 2)
					Sys_LoadMenu();
		...
		else if (btn3.GetState() == STATE_CLICKED)
		{
			...
			{
				if(isWiiU())
					ret = WindowPrompt(tr("Power"), 0, tr("Shutdown"), tr("Cancel"));
				else
					ret = WindowPrompt(tr("Power"), 0, tr("Shutdown"), tr("Standby"), tr("Cancel"));
...
int FormatingPartition(const char *title, int part_num)
{
	...
	int choice = WindowPrompt(tr("Format"), text, tr("Yes"), tr("No"));
	if (choice == 0)
		return -666;
	...
	if (ret < 0)
	{
		WindowPrompt(tr("Error"), tr("An error occurred while formating"), tr("Return"));
	}
	else
	{
		...
		snprintf(text + n, sizeof(text) - n, " %s", tr("formatted successfully"));
		WindowPrompt(tr("Format"), text, tr("OK"));
		if (ret < 0)
		{
			WindowPrompt(tr("Error"), tr("An error occurred while opening partition"), tr("OK"));
			Sys_LoadMenu();
		}
	}
...
int CodeDownload(const char *id)
{
	...
	{
		WindowPrompt(tr("File Error"), tr("An error occurred while creating directory"), tr("OK"));
		return -1;
	}
	...
	while (!IsNetworkInit())
	{
		...
		{
			msgTxt.SetText(tr( "An error occurred while initializing network" ));
		}
		if (btn1.GetState() == STATE_CLICKED)
		{
			btn1.ResetState();
	...
	if (IsNetworkInit())
	{
		...
		if (file.size > 7)
		{
			...
			if (!validUrl)
			{
				snprintf(codeurl, sizeof(codeurl), "%s.txt%s", id, tr(" not found on server"));
			...
			else
			{
				...
				if (pfile)
				{
					...
					if (pfile)
					{
						...
						{
							snprintf(txtpath + txtLen, sizeof(txtpath) - txtLen, "%s", tr(" saved successfully. The text has not been verified. Some of the code may not work right with each other" ));
					...
					WindowPrompt(tr("Error"), tr("An error occurred while writing file"), tr("OK"));
			}
			MEM2_free(file.data);
		}
		else
