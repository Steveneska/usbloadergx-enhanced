...
int GCMultiDiscMenu::ShowSelection()
{
	while(true)
	{
		...
		{
			if(!changed || WindowPrompt(tr("Install Wizard"), tr("Discard changes?"), tr("Yes"), tr("No")))
				break;
		...
		else if(button1->GetState() == STATE_CLICKED)
		{
			if(!changed)
			{
				WindowPrompt(tr("File Error"), tr("No games selected to install"), tr("OK"));
			}
			else if(WindowPrompt(tr("Install Wizard"), tr("Install all selected games?"), tr("Yes"), tr("Cancel")))
			{
				return 1;
			}
