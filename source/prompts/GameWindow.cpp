	...
	, browserMenu(m)
{
	...
	detailsBtnOverTxt = new GuiText(tr( "Details" ), 22, thColor("r=0 g=0 b=0 a=255 - game window details button text color"));
	detailsBtn = new GuiButton(detailsBtnTxt->GetTextWidth(), 25);
	detailsBtn->SetAlignment(ALIGN_CENTER, ALIGN_MIDDLE);
	detailsBtn->SetPosition(140, -75);
...
void GameWindow::ChangeGame(int EffectDirection)
{
	...
	if (header->tid != 0)
	{
		if(header->type == TYPE_GAME_NANDCHAN)
			sizeTxt->SetTextf(tr("NAND"));
		else if(header->type == TYPE_GAME_EMUNANDCHAN)
			sizeTxt->SetTextf(tr("EmuNAND"));
...
void GameWindow::BootGame(struct discHdr *header)
{
	...
	if (game_cfg->loadalternatedol == 2)
	{
		...
		{
			snprintf(filepath + n, sizeof(filepath) - n, " %s", tr("not found" ));
	...
	if (game_cfg->ocarina == ON || (game_cfg->ocarina == INHERIT && Settings.ocarina == ON))
	{
		...
		if (CheckFile(filepath) == false)
		{
			snprintf(filepath + n, sizeof(filepath) - n, " %s", tr("not found. Loading game without cheats" ));
			if (!WindowPrompt(tr("Error"), filepath, tr("Continue"), tr("Cancel")))
				return;
		}
	}
	...
	if (autoIOS == GAME_IOS_CUSTOM)
	{
		if (header->type == TYPE_GAME_EMUNANDCHAN)
		{
			if (gameNandEmuMode != EMUNAND_NEEK)
			{
				...
				{
					ShowError(tr("Launching EmuNAND channels only works with a D2X cIOS. Set game IOS to D2X cIOS first"));
					return;
				}
			}
		}
		...
		if (header->type == TYPE_GAME_WII_IMG || header->type == TYPE_GAME_WII_DISC)
		{
			...
			{
				ShowError(tr("Launching Wii games from EmuNAND only works with a D2X cIOS. Set game IOS to D2X cIOS first"));
				return;
			}
		}
	}
