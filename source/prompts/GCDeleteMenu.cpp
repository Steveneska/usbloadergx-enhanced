...
{
	titleTxt->SetText(tr("Delete GameCube Games"));
...
int GCDeleteMenu::Show()
{
	while(true)
	{
		...
		{
			if(!changed || WindowPrompt(tr("Delete Games"), tr("Discard changes?"), tr("Yes"), tr("No")))
				break;
		...
		else if(button1->GetState() == STATE_CLICKED)
		{
			if(!changed)
			{
				WindowPrompt(tr("File Error"), tr("No games selected to delete"), tr("OK"));
			}
			else if(WindowPrompt(tr("Delete Games"), tr("Delete all selected games from SD Card?"), tr("Yes"), tr("Cancel")))
			{
				DeleteSelectedGames();
				break;
			}
