	...
	: GuiWindow(0, 0)
{
	...
	addTxt = new GuiText(tr("Add new"), 24, thColor("r=0 g=0 b=0 a=255 - prompt windows button text color"));
	...
	deleteTxt = new GuiText(tr("Delete"), 24, thColor("r=0 g=0 b=0 a=255 - prompt windows button text color"));
	...
	editTxt = new GuiText(tr("Rename"), 24, thColor("r=0 g=0 b=0 a=255 - prompt windows button text color"));
...
int CategoryPrompt::Show()
{
	while(backBtn->GetState() != STATE_CLICKED)
	{
		...
		else if(deleteButton->GetState() == STATE_CLICKED)
		{
			...
			{
				WindowPrompt(tr("Error"), tr("Default category cannot be deleted"), tr("OK"));
				mainWindow->SetState(STATE_DISABLED);
				SetState(STATE_DEFAULT);
				deleteButton->ResetState();
				continue;
			...
			int choice = WindowPrompt(tr("Delete Category"), tr("This will not delete any games currently assigned to this category"), tr("Yes"), tr("Cancel"));
			if(choice)
			{
