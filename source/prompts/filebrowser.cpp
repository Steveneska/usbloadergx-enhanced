...
int BrowseDevice(char * Path, int Path_size, int Flags, FILTERCASCADE *Filter/*=NULL*/)
{
	...
	{
		WindowPrompt(tr("Error"), tr("Something went wrong"), tr("OK"));
		return -1;
	}
	int menu = MENU_NONE;
	...
	while (menu == MENU_NONE)
	{
		...
		else if (folderBtn.GetState() == STATE_CLICKED)
		{
			...
			if (result == 1)
			{
				...
				{
					if (WindowPrompt(tr("Error"),
							tr("Directory does not exist. Create directory?"),
							tr("OK"), tr("Cancel")) == 1) if (CreateSubfolder(newfolder) == false) WindowPrompt(
							tr("File Error"), tr("An error occurred while creating directory"), tr("OK"));
				}
