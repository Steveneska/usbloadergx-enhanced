...
int ThemeMenu::MainLoop()
{
	...
	{
		int choice = WindowPrompt(tr("Theme Manager"), tr("Load USB Loader GX default theme?"), tr("Yes"), tr("Cancel"));
		if(choice)
		{
			HaltGui();
			Theme::SetDefault();
...
void ThemeMenu::SetupMainButtons()
{
	...
	{
		WindowPrompt(tr("File Error"), tr("Theme files not found"), tr("OK"));
	}
...
void ThemeMenu::MainButtonClicked(int button)
{
	...
	GuiText titleTxt(tr("Theme"), 18, thColor("r=0 g=0 b=0 a=255 - prompt windows text color"));
	...
	versionTxt2.SetPosition(230, PositionY);
	versionTxt2.SetMaxWidth(dialogBox.GetWidth() - 220, DOTTED);

	if(versionTxt2.GetTextWidth() >= dialogBox.GetWidth() - 220)
		PositionY += 50;
	else
		PositionY += 30;
	...
	GuiButton applyBtn(&applyBtnImg, &applyBtnImg, ALIGN_LEFT, ALIGN_TOP, 0, 220, &trigA, btnSoundOver, btnSoundClick2, 1); // -5 170
	...
	while (!leave)
	{
		...
		{
			int choice = WindowPrompt(tr("Theme Manager"), tr("Load theme"), title, tr("Yes"), tr("Cancel"));
			if (choice)
			{
