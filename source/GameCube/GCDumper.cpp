...
s32 GCDumper::InstallGame(const char *installpath, u32 game, const char *installedGamePath)
{
	...
	//! Check for enough free space
	{
		...
		{
			ShowError(tr("Device free space could not be calculated for installation"));
			return -102;
		}

		if(((u64)sd_vfs.f_frsize * (u64)sd_vfs.f_bfree) < discTotal)
		{
			ShowError(tr("Not enough free space to install game on this device"));
			return -103;
		}
	}
	...
	u8 *FSTBuffer = (u8 *)memalign(32, ALIGN32(FSTSize));
	if(!FSTBuffer) {
		ShowError(tr("Not enough free space for disc FST offset"));
		return -3;
	}
	...
	{
		free(FSTBuffer);
		ShowError(tr("Something wrong %s"), gamepath);
		return -4;
	}
	...
	gprintf("App loader size: %d\n", ApploaderSize);
	gprintf("DOL offset: 0x%08x\n", DOLOffset);
	gprintf("DOL size: %d\n", DOLSize);
	gprintf("FST offset: 0x%08x\n", FSTOffset);
	gprintf("FST size: %d\n", FSTSize);
	gprintf("Number of FST entries: %d\n", FSTEnt);
	...
	if(compressed)
	{
		...
		gprintf("Old disc size: %d - New disc size: %d - Saved: %d\n", DiscSize, wrote, DiscSize - wrote);
	}
	else
	{
