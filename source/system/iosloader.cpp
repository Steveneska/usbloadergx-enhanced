...
void IosLoader::GetD2XInfo()
{
	...
	for (s32 i = 255; i >= 200; i--) // Prefer higher slots e.g. 251, 250 & 249
	{
		if (IsD2XBase(i, &base))
		{
			...
			gprintf("D2X cIOS %d (base %d) found\n", i, base);
		}
	}
	ISFS_Deinitialize();
...
void IosLoader::LoadIOSModules(s32 ios, s32 ios_rev)
{
	...
	if(IsHermesIOS(ios))
	{
		...
		gprintf("Loading EHC 5 and OpenDip module\n");
	...
	else if(IsWaninkokoIOS(ios))
	{
		...
		if(ios_rev >= 18 && (!info || info->version < 6))
		{
			if(mload_init() >= 0)
			{
				gprintf("Loading dip module for Waninkoko cIOS\n");
