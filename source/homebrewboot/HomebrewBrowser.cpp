...
int HomebrewBrowser::MainLoop()
{
	...
	{
		if(IsNetworkInit() && WindowPrompt(tr("Network"), tr("Initialize network connection?"), tr("Yes"), tr("Cancel")))
		{
			DeinitNetwork();
			wifiBtn->SetAlpha(80);
...
int HomebrewBrowser::ReceiveFile()
{
	...
	snprintf(temp, sizeof(temp), tr("Load file from %s IP address?" ), GetIncommingIP());
	...
	while (read < infilesize)
	{
		...
		if (result < 0)
		{
			WindowPrompt(tr("Process Error"), tr("An error occurred while transfering data"), tr("OK"));
			free(buffer);
			ProgressStop();
			return MENU_NONE;
		}
	...
	if (wiiloadVersion[0] > 0 || wiiloadVersion[1] > 4)
	{
		...
		if (buffer[0] == 'P' && buffer[1] == 'K' && buffer[2] == 0x03 && buffer[3] == 0x04)
		{
			...
			if (uf == NULL)
			{
				WindowPrompt(tr("File Error"), tr("An error occurred while opening ZIP file"), tr("OK"));
				return MENU_NONE;
			}
			...
			WindowPrompt(tr("Install Wizard"), tr("ZIP file successfully installed to homebrew directory"), tr("OK"));
	...
	{
		WindowPrompt(tr("File Error"), tr("No data to be read"), tr("OK"));
		FreeHomebrewBuffer();
		return MENU_NONE;
	}
