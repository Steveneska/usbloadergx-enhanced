...
bool SystemMenuResources::Init()
{
	...
	if( !p_tmd )
	{
		gprintf("Could not find System Menu TMD file\n");
		return false;
	}
	...
	if( idx == 0xffff )
	{
		gprintf("System Menu main resource not found\n" );
		return false;
	}
	// Build file path
	...
	{
		gprintf("Error while reading resource from NAND: %i\n", ret);
		return false;
	}
	...
	if(!chanTtlAsh)
	{
		gprintf("Error while loading chanTtl.ash file\n");
	}
	// Move this to MEM2
	...
	if(!chanSelAsh)
	{
		gprintf("Error while loading chanSel.ash file\n");
	}
	// Move this to mem2
...
bool SystemMenuResources::InitFontArchive(void)
{
	...
	if(!contentMap)
	{
		gprintf("Could not find contentMap file\n");
		return false;
	}
...
bool SystemMenuResources::InitSystemFontArchive(bool korean, u8 *contentMap, u32 mapsize)
{
	...
	for (int i = 0; i < fileCount; i++)
	{
		...
		free(fontArchive);
		gprintf("Wii System Menu font loaded\n");
		return true;
	}
