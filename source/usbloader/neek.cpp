...
{
	gprintf( "Loading NEEK kernel.bin");
	char kernelPath[30];
	if(isWiiU())
	...
	gprintf("Kernel loaded to 0x%08x, size: %d\n", kernel, fsize);
	gprintf("NEEK memory address offset: %08x\n", (u32)kernel - 0x80000000);	// offset
...
int neek2oSetNAND(const char* nandpath)
{
	...
	if(!CheckFile(nandconfigPath) || FileSize(nandconfigPath) < NANDCONFIG_HEADER_SIZE+1)
	{
		...
		if(!f)
		{
			gprintf("Could not create file %s\n", nandconfigPath);
			return -1;
		}
	...
	if(ret != filesize)
	{
		gprintf("Could not load file %s to memory\n", nandconfigPath);
		fclose (f);
		MEM2_free(nandCfg);
		return -1;
	}
	...
	else // New NAND path to nandcfg.bin
	{
		...
		if(newNand)
		{
			...
			if(ret != sizeof(NandInfo))
				gprintf("Could not write new NAND info\n");
