...
int CheatMenu(const char * gameID)
{
	...
	{
		case -1:
			blankchoice = WindowPrompt(tr("File Error"), tr("The selected cheat file content is blank"), tr("Delete"), tr("OK"));
			if(blankchoice)
			{
				char gctPath[200];
			...
			break;
		case 0:
			download = WindowPrompt(tr("File Error"), tr("Game cheat file not found"), tr("Download"), tr("Cancel"));
			if (download == 1)
			{
			...
			while (!exit)
			{
				...
				if (createBtn.GetState() == STATE_CLICKED)
				{
					if (cntcheats > 0)
					{
						...
						{
							if(WindowPrompt(tr("Error"), tr("No cheat codes selected. Delete GCT cheat file instead?"), tr("Yes"), tr("Cancel")))
							{
								RemoveFile(gctPath);
								w.Remove(&chtBrowser);
						...
						else
						{
							...
							WindowPrompt(tr("Cheat Manager"), tr("GCT cheat file created successfully"), tr("OK"));
						}
					}
					else
						WindowPrompt(tr("File Error"), tr("An error occurred while creating GCT cheat file"), tr("OK"));
