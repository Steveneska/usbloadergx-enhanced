...
#ifdef DEBUG_NETWORK
	gprintf("Socket closed and cleaned up\n");
#endif
}
...
void downloadfile(const char *url, struct download *buffer)
{
	...
	if (httpinfo.sock < 0)
	{
		...
		if (httpinfo.sock == -ETIMEDOUT)
			...
			gprintf("\nConnection timed out: could not connect to %i\n", httpinfo.sock);
#endif
		return;
	}
#ifdef DEBUG_NETWORK
	...
	if (getProxyAddress() && getProxyPort() > 0)
	{
		...
		{
#ifdef DEBUG_NETWORK
			gprintf("Could not connect to proxy %s:%i\n", getProxyAddress(), getProxyPort());
#endif
...
#ifdef DEBUG_NETWORK
		gprintf("Proxy ready to receive data\n");
#endif
	}
	...
	if (httpinfo.use_https)
	{
		...
		{
#ifdef DEBUG_NETWORK
			gprintf("WOLFSSL_CTX could not be created\n");
#endif
			https_close(&httpinfo);
			return;
		}
		...
		{
#ifdef DEBUG_NETWORK
			gprintf("SSL session could not be established\n");
#endif
			https_close(&httpinfo);
			return;
		}
		...
		{
#ifdef DEBUG_NETWORK
			gprintf("SSL file descriptor could not be set\n");
#endif
			https_close(&httpinfo);
			return;
		}
		...
		{
#ifdef DEBUG_NETWORK
			gprintf("Connection error: session timed out\n");
#endif
			session = NULL;
		}
		...
		while (true)
		{
			...
			{
#ifdef DEBUG_NETWORK
				gprintf("Could not set SSL handshake\n");
#endif
				https_close(&httpinfo);
				return;
			}
		...
		{
#ifdef DEBUG_NETWORK
			gprintf("Connection error: session could not be resumed\n");
#endif
			session = NULL;
		}
		// Cipher info
	...
	{
#ifdef DEBUG_NETWORK
		gprintf("Request sent to %s. Skipping response\n", host);
#endif
		https_close(&httpinfo);
		return;
	}
	...
	if (response.status == 301 || response.status == 302)
	{
		...
		{
#ifdef DEBUG_NETWORK
			gprintf("Redirect limit reached\n");
#endif
			return;
		}
		loop++;
...
#ifdef DEBUG_NETWORK
			gprintf("Incomplete download removed\n");
#endif
			https_close(&httpinfo);
			return;
		}
