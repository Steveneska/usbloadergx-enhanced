...
u8 *Channels::GetDol(const u64 &title, u8 *tmdBuffer, bool &isForwarder)
{
	...
	{
		gprintf("An error occurred while loading channel DOL\n");
		free(filepath);
		return NULL;
	}
	...
	if (isLZ77compressed(buffer))
	{
		...
		{
			gprintf("Boot content could not be decompressed\n");
			free(buffer);
			return NULL;
		}
		free(buffer);
...
u8 *Channels::GetTMD(const u64 &tid, u32 *size, const char *prefix)
{
	...
	if (ret < 0)
	{
		gprintf("An error occurred while reading TMD file\n");
		if (tmdBuffer)
			free(tmdBuffer);
		return NULL;
	}
...
u32 Channels::LoadChannel(const u64 &chantitle)
{
	...
	if (dolfile->bss_start)
	{
		...
		{
			// For homebrew...not all have it clean
...
bool Channels::Identify(const u64 &titleid, u8 *tmdBuffer, u32 tmdSize)
{
	...
	{
		free(filepath);
		gprintf("An error occurred while generating ticket file");
		return false;
	}
	...
	{
		gprintf("An error occurred while reading system certificates\n");
		free(tikBuffer);
		free(filepath);
		return false;
	}
	...
	if (ret < 0)
	{
		...
		{
			case ES_EINVAL:
				gprintf("ES_Identify Error (ret = %d;) - Data invalid\n", ret);
				break;
			case ES_EALIGN:
				gprintf("ES_Identify Error (ret = %d;) - Data not aligned\n", ret);
				break;
			case ES_ENOTINIT:
				gprintf("ES_Identify Error (ret = %d;) - ES not initialized\n", ret);
				break;
			case ES_ENOMEM:
				gprintf("ES_Identify Error (ret = %d;) - No memory\n", ret);
				break;
			default:
				gprintf("ES_Identify Error (ret = %d)\n", ret);
				break;
		}
	}
