...
bool Wad::Open(const char *wadpath)
{
	...
	{
		if(showPrompt)
			ShowError(tr("An error occurred while opening %s"), wadpath);
		return false;
	}
	...
	{
		if(showPrompt)
			ShowError(tr("An error occurred while reading WAD header"));
		return false;
	}
	...
	{
		if(showPrompt)
			ShowError(tr("An error occurred while reading ticket"));
		return false;
	}
	...
	{
		if(showPrompt)
			ShowError(tr("An error occurred while reading TMD file"));
		return false;
	}
...
bool Wad::WriteFile(const char *filepath, u8 *buffer, u32 len)
{
	...
	{
		if(showPrompt)
			ShowError(tr("An error occurred while creating %s"), filepath);
		return false;
	}
	...
	if(write != len && showPrompt)
		ShowError(tr("An error occurred while saving %s"), filepath);
...
bool Wad::InstallContents(const char *installpath)
{
	...
	for (cnt = 0; cnt < tmd_data->num_contents; cnt++)
	{
		...
		{
			if(showPrompt)
				ShowError(tr("File read/write process stopped unexpectedly"));
			return false;
		}
...
int Wad::CheckContentMap(const char *installpath, tmd_content *content, char *filepath)
{
	if(!content_map)
	{
		...
		{
			if(showPrompt)
				ShowError(tr("An error occurred while reading %s"), filepath);
			return -1;
		}
